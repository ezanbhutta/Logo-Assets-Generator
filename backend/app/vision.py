...
_SYSTEM = (
    ...
    "swatches, a tagline, and a paragraph of descriptive copy. Like a brand "
    "designer, find the ONE clean primary logo lockup and the standalone icon, "
    "and ignore everything that is presentation scaffolding."
)
...
_PROMPT = (
    ...
    "logo (the wordmark plus any symbol that is part of it). Enclose ONE clean "
    "instance tightly, and INCLUDE any trailing period, dot, or sparkle that is "
    "part of the wordmark. Prefer the version on a plain light background. "
    "EXCLUDE background tiles/panels, color swatches, taglines, descriptive "
    "paragraphs, and duplicate or reversed/alternate-color versions. If the "
    "artboard is already just the single logo with nothing else to exclude, use "
    "null (meaning: the whole artwork is the logo).\n"
    '- "icon_box": the standalone icon / brandmark — a compact symbol or '
    "monogram shown by itself (for example a single letter with a small symbol). "
    "Use null if there is no distinct standalone icon (a plain wordmark has "
    "none).\n"
